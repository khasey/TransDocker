...
import { CreateChannelDto, AddMembersDto, AddAdminsDto, AddOwnerDto , AddBanUserDto , AddKickUsersDto} from './channel.dto';
...
async banUserFromChannel(@Param('name') name: string, @Body() banUserDto: AddBanUserDto): Promise<Channel> {
  ...
  try{
  return this.channelService.banUserFromChannel(name, banUserDto.banned);
  } catch (error) {
