...
// ============ Update des membres ============
  ...
  async banUserFromChannel(channelName: string, banned: number, banneur? : number): Promise<Channel> {
    ...
    // Si le tableau banned n'existe pas encore, créez-le
    if (!channel.banned) {
      channel.banned = [];
    }

    // Ajoutez l'utilisateur à bannir seulement s'il n'est pas déjà dans la liste
    if (!channel.banned.includes(banned)) {
      channel.banned.push(banned);
    }

	console.log(	'ownerId ===>' + banneur);
	// Supprimez l'utilisateur du tableau des admins si l'utilisateur qui lance la commande est owner
	if (channel.owner.includes(banneur)) {
		console.log(	'ownerId test 2 ===>' + banneur);
    ...
    // Supprimez l'utilisateur du tableau des membres
...
async kickUserFromChannel(name: string, userId: number): Promise<Channel> {
  const channel = await this.getChannelByName(name);
  ...
  return channel;
   ...
   async muteUserFromChannel(name: string, muted: number, muteur? : number): Promise<Channel> {
	const channel = await this.getChannelByName(name);
