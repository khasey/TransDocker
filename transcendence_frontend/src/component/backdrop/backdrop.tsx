...
useEffect(() => {
	...
	socket.on('chat message', (message: Message) => {
	  ...
	  if (selectedChannel && selectedChannel.members.includes(message.user?.id)) {
		setMessages((prevMessages) => [...prevMessages, message]);
	  }
	});

	socket.on('chat messages', (messages: Message[]) => {
	  // Filtrer les messages pour n'inclure que ceux dont l'utilisateur est toujours membre du channel
	//  if (selectedChannel) {
	//	const filteredMessages = messages.filter((message) =>
	//	  selectedChannel.members.includes(message.user?.id)
	//	);
