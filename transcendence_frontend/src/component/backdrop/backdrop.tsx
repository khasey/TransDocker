...
export default function Backdrop() {
  ...
  const [selectedTab, setSelectedTab] = useState<Tab>({ id: 1, label: 'General' });
              ...
              <div className={styles.inside_chat}>
                ...
                <div className={styles.channel_tchat}>
                  <div className={styles.title}>
                    <FullChannel selectedTab={selectedTab} />
                  </div>
                  <div className={styles.tchat}>
                    <div className={styles.textsendermodule}>
                    {messages.filter(message => Number(message.channelId) === selectedTab.id).map((message) => (
                    <TextSend
                      message={message}
                      // user={message.user}
                      // id={message.id}
                ...
                <LabelBottomNavigation setValue={setSelectedTab}/>
              </div>
            </SwipeableDrawer>
          </React.Fragment>
        ))}
