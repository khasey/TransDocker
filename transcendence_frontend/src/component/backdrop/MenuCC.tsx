...
export const MenuCC: React.FC<MenuCCProps> = ({ handleClose }) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (response.data.owner.includes(userId)) {
          console.log('new private ===>'+newIsPrivate)
  ...
  console.log('searched dans handle avant ====>>>>' + name)
  ...
  const handleClickCreate = async () => {
    try {
      ...
      // Si le canal n'existe pas (c'est-à-dire, l'API renvoie une erreur 404), créer un nouveau canal
  ...
  async function checkChannelExists(searchedchannel: string, userId?: number) {
	console.log("Checking if channel exists: ", searchedchannel);
	try {
